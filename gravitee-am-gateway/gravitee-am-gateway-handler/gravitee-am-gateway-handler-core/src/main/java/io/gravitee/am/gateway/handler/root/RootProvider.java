...
import io.gravitee.am.gateway.handler.common.vertx.web.handler.GraviteeContextHandler;
...
public class RootProvider extends AbstractService<ProtocolProvider> implements ProtocolProvider {
    ...
    protected void doStart() throws Exception {
        ...
        graviteeContextHandler(rootRouter);
        ...
        // logout route
        rootRouter.route(PATH_LOGOUT).handler(new LogoutEndpoint(domain, tokenService, auditService, authenticationFlowContextService));
    ...
    private void graviteeContextHandler(Router router) {
        // Login endpoint
        GraviteeContextHandler graviteeContextHandler = new GraviteeContextHandler(authenticationFlowContextService);
        router.route(PATH_LOGIN).handler(graviteeContextHandler);
        router.route(PATH_LOGIN_CALLBACK).handler(graviteeContextHandler);
        router.route(PATH_LOGIN_SSO_POST).handler(graviteeContextHandler);

        // MFA endpoint
        router.route(PATH_MFA_ENROLL).handler(graviteeContextHandler);
        router.route(PATH_MFA_CHALLENGE).handler(graviteeContextHandler);

        // Registration confirmation endpoint
        router.post(PATH_REGISTER).handler(graviteeContextHandler);
        router.route(PATH_CONFIRM_REGISTRATION).handler(graviteeContextHandler);

        // Reset password endpoint
        router.route(PATH_RESET_PASSWORD).handler(graviteeContextHandler);

        // WebAuthn endpoint
        router.route(PATH_WEBAUTHN_REGISTER).handler(graviteeContextHandler);
        router.route(PATH_WEBAUTHN_RESPONSE).handler(graviteeContextHandler);
        router.route(PATH_WEBAUTHN_LOGIN).handler(graviteeContextHandler);
    }
