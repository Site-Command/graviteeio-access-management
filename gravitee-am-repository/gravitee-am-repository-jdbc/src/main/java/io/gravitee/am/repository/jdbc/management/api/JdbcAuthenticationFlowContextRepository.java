...
import io.gravitee.am.repository.jdbc.management.api.model.JdbcAuthenticationContext;
...
public class JdbcAuthenticationFlowContextRepository extends AbstractJdbcRepository implements AuthenticationFlowContextRepository {
    ...
    protected AuthenticationFlowContext toEntity(JdbcAuthenticationContext entity) {
    ...
    public Maybe<AuthenticationFlowContext> findById(String id) {
        ...
        return monoToMaybe(dbClient.select()
                .from(JdbcAuthenticationContext.class)
                .matching(from(where("id").is(id)))
                .as(JdbcAuthenticationContext.class).one())
                .map(this::toEntity)
                .doOnError((error) -> LOGGER.error("Unable to retrieve AuthenticationContext with id {}", id, error));
    }

    @Override
    public Maybe<AuthenticationFlowContext> findLastBySession(String session) {
        LOGGER.debug("findLastBySession({})", session);
        LocalDateTime now = LocalDateTime.now(UTC);
        return monoToMaybe(dbClient.select()
                .from(JdbcAuthenticationContext.class)
                .matching(from(where("session_id").is(session).and(where("expire_at").greaterThan(now))))
                .orderBy(Sort.Order.desc("version"))
                .as(JdbcAuthenticationContext.class).first())
                .map(this::toEntity)
                .doOnError((error) -> LOGGER.error("Unable to retrieve AuthenticationContext with session {}", session, error));
    }
    ...
    public Flowable<AuthenticationFlowContext> findBySession(String session) {
        ...
        return fluxToFlowable(dbClient.select()
                ...
                .matching(from(where("session_id").is(session).and(where("expire_at").greaterThan(now))))
                .orderBy(Sort.Order.desc("version"))
                .as(JdbcAuthenticationContext.class).all())
                .map(this::toEntity)
                .doOnError((error) -> LOGGER.error("Unable to retrieve all AuthenticationContext with session {}", session, error));
    }

    @Override
    public Single<AuthenticationFlowContext> create(AuthenticationFlowContext context) {
       String id = context.getSessionId() + "-" + context.getVersion();
        LOGGER.debug("Create AuthenticationContext with id {}", id);

        DatabaseClient.GenericInsertSpec<Map<String, Object>> insertSpec = dbClient.insert().into("authentication_contexts");

        // doesn't use the class introspection to handle json objects
        insertSpec = addQuotedField(insertSpec,"id", id, String.class);
        insertSpec = addQuotedField(insertSpec,"session_id", context.getSessionId(), String.class);
        ...
        return monoToSingle(insertAction)
                ...
                .doOnError((error) -> LOGGER.error("unable to create authentication context with id {}", id, error));
    }
    ...
    public Completable delete(String session) {
        ...
        return monoToCompletable(dbClient.delete()
                .from(JdbcAuthenticationContext.class)
                .matching(from(where("session_id").is(session))).fetch().rowsUpdated());
    }

    @Override
    public Completable delete(String session, int version) {
        LOGGER.debug("delete({}, {})", session, version);
        return monoToCompletable(dbClient.delete()
                .from(JdbcAuthenticationContext.class)
                .matching(from(where("session_id").is(session).and(where("version").is(version)))).fetch().rowsUpdated());
    }
    ...
    public Completable purgeExpiredData() {
        ...
        LocalDateTime now = LocalDateTime.now(UTC);
        return monoToCompletable(dbClient.delete().from(JdbcAuthenticationContext.class).matching(where("expire_at").lessThan(now)).then()).doOnError(error -> LOGGER.error("Unable to purge authentication contexts", error));
    }
}
