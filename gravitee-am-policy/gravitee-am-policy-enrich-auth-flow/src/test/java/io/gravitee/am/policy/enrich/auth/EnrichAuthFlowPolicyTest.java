...
public class EnrichAuthFlowPolicyTest {
    ...
    public void init() {
        ...
        when(configuration.getDuration()).thenReturn(1l);
        when(configuration.getTimeUnit()).thenReturn("MINUTES");
    }

    private void mockGraviteeContext(int version, Map<String, Object> data) {
        GraviteeContext gioContext = new GraviteeContext();
        ...
        gioContext.setAuthenticationFlowContext(authenticationFlowContext);
        when(executionContext.getAttribute(GraviteeContext.ROUTING_CONTEXT_FIELD_NAME)).thenReturn(gioContext);
    }
